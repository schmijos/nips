...
```json
{
  ...
  "limit": <maximum number of matched events relays SHOULD return in the initial query>
}
```
...
Filter attributes containing lists (`ids`, `authors`, `kinds` and tag selectors like `#e`) are JSON arrays with one or more values. At least one of the arrays' values must match the relevant field in an event for the condition to be considered a match. Scalar event attributes such as `pubkey` and `kind` must match the corresponding filter condition exactly. In the case of tag attributes such as `#e`, for which an event may have multiple values, the event and filter condition values must have at least one item in common.
